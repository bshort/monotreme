...
func (d *DB) CreateCollection(ctx context.Context, create *storepb.Collection) (*storepb.Collection, error) {
	...
	args := []any{create.CreatorId, create.Name, create.Title, create.Description, pq.Array(create.ShortcutIds), create.Visibility.String(), create.CustomIcon, create.UserOrder, create.Uuid}
...
func (d *DB) UpdateCollection(ctx context.Context, update *store.UpdateCollection) (*storepb.Collection, error) {
	...
	if update.UserOrder != nil {
		set, args = append(set, "user_order = "+placeholder(len(args)+1)), append(args, *update.UserOrder)
	}
	if update.UUID != nil {
		set, args = append(set, "uuid = "+placeholder(len(args)+1)), append(args, *update.UUID)
	}
	if len(set) == 0 {
