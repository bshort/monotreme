...
const Favorites: React.FC = () => {
  ...
  const [, setLastVisited] = useLocalStorage<string>("lastVisited", "/favorites");
  ...
  // Merge URL tags with current filter and enable favorites filter
  const filter = {
    ...viewStore.filter,
  ...
  useEffect(() => {
    setLastVisited("/favorites");
    viewStore.setRoute("/favorites");
    Promise.all([shortcutStore.fetchShortcutList()]).finally(() => {
      loadingState.setFinish();
    });
  }, []);

  // Ensure favorites filter is always enabled on this page
  useEffect(() => {
    if (!viewStore.filter.showFavoritesOnly) {
      viewStore.setFilter({ showFavoritesOnly: true });
    }
  }, [viewStore.filter.showFavoritesOnly]);
  ...
  const handleTagClick = (tag: string) => {
    window.location.href = `/favorites?tags=${encodeURIComponent(tag)}`;
  };
      ...
      <div className="mx-auto max-w-8xl w-full px-4 sm:px-6 md:px-12 pt-4 pb-6 flex flex-col justify-start items-start">
        ...
        {loadingState.isLoading ? (
          ...
          <div className="py-16 w-full flex flex-col justify-center items-center text-gray-400">
            ...
            <p className="mt-2">No favorite bookmarks found.</p>
            <p className="mt-1 text-sm">Click the star icon on any bookmark to add it to your favorites.</p>
          </div>
        ) : (
